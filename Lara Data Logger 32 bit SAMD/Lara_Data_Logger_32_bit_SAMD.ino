...
volatile bool dataRdyIntSet = false;
volatile bool gyroDataRdy = false;
...
volatile float beta = .8;                                // algorithm gain
...
volatile bool wdt_int_occured = false;
...
void loop()
{
    ...
    if(!dataRdyIntSet) {
        ...
        //initialize data to store in FIFO
    ...
    while((Remainder[0] > 0 || Remainder[1] > 0) && fifoCount[0] < 512 ) {  //wait for
        errorCounter++;
        ...
        if(fifoCount[0] >= 512) {
            int i;
            //push put the 8 extra bytes so we are synced again
            for (i=0; i<8; i++) {
                IMU1.getFIFOByte();
            }
        }
...
void IMU_int() {
   ...
   // millisecondsFIFORead = (uint32_t)millis();
}
